# ----------------------------------------------------------- #
# Function: Extract the start of the tics
# Author: Martyna Siatka
# Goal: Exctract the start of the tic based on the key pressed by the participants and manual annotations
# ----------------------------------------------------------- #

"""
For each phase there were different instruction:
- spontaneous: participant has to press KEY_D for the start of the urge and KEY_F for the end of the urge
- imitated: participant has to press KEY_D for the start of the imitated tic and KEY_F for the end of the imitated tic and KEY_T to indicate that real tic occured
- suppressed: participant has to press KEY_S for the start of the suppressed tic and KEY_F for the end of the suppressed tic and KEY_T to indicate real tic occuring
"""
...
def analyse_merged_ttl_tics_spontaneous(merged_ttl_tics,phase_start_key='start_spont', phase_end_key='end_spont'):
    """
    We are looking for the KEY_D pressed to indicate the start of the tic. However, if the start from the EXCEL annotations preceds the KEY_D then the start is the EXCEL start.
    """
    ...
    print("[DEBUG] Start and end of the phase", t_start, t_end)
    ...
    while i < len(phase_events):
        ...
        if key == 'KEY_D':
            ...
            for j in range(i-1, -1, -1):
                ...
                if k.startswith('start_') and v not in start_used:
            ...
            if found_back is not None and found_back[0] == 'KEY_D':
            ...
            # Case 3 : D begins (end_i before D)
                ...
                for j_forward in range(i+1, len(phase_events)):
                    ...
                    if k_fwd.startswith('start_') and v_fwd not in start_used:
                        found_forward = ('start_i', v_fwd)
                        break
                    elif k_fwd == 'KEY_F':
                        found_forward = ('KEY_F', v_fwd)
                        break

                if found_forward is not None:

                    # Case 3.a. : D then start
                    if found_forward[0] == 'start_i' and found_forward[1] not in start_used:
...
def analyse_merged_ttl_tics_imitated(merged_ttl_tics,
    ...
    while i < len(phase_events):
        ...
        if key == 'KEY_D':
            ...
            for j_forward in range(i+1, len(phase_events)):
                ...
                if k_fwd == 'KEY_F':
                    ...
                    found_forward = ('KEY_T', v_fwd)
                    break

            # ----- D then D -----
            if found_forward is not None and found_forward[0] == 'KEY_D':
                # results_list.append({
                #     "type": "D_then_D",
                #     "D_time": value,
                # })
            ...
            # ----- D then F -----
            if found_forward is not None and found_forward[0] == 'KEY_F':
                if found_forward[1] not in F_used and value not in D_used:
                    # check if there's a T just before this D
                    t_before_d = None
                    for j in range(i-1, -1, -1):
                        k, v = next(iter(phase_events[j].items()))
                        if k == 'KEY_T':
                            t_before_d = v
                            break
                        elif k in ('KEY_D', 'KEY_F', 'start_', 'end_'):
                            break

                    if t_before_d is not None and value - t_before_d <= max_t_before_D:
                        # T before D → it's T_then_D_then_F
                        results_list = [r for r in results_list if not (r.get('type') == 'T_before_D' and r.get('T_time') == t_before_d)]
                        results_list.append({
                            "type": "T_then_D_then_F",
                            "T_time": t_before_d,
                        })
                        D_used.add(value)
                        F_used.add(found_forward[1])
                        print(f"T at {t_before_d} then D at {value} then F at {found_forward[1]}")
                    else:
                        # no T before → regular D_then_F
                        results_list.append({
                            "type": "D_then_F",
                            "D_time": value,
                        })
            ...
            # ------ D after T ------
            # elif found_back is not None and found_back[0] == 'KEY_T':
            ...
            # ----- D then F -----
            # elif found_forward is not None and found_forward[0] == 'KEY_F':
            #     if found_forward[1] not in F_used and value not in D_used:
            #         results_list.append({
            #             "type": "D_then_F",
            #             "D_time": value,
        ...
        if key == 'KEY_T':
            ...
            # --- T after T ---
            if found_back is not None and found_back[0] == 'KEY_T':
            ...
            # ---- T after F ----
            if found_back is not None and found_back[0] == 'KEY_F':
        ...
        if key == 'KEY_F' :
            ...
            for j in range(i + 1, len(phase_events)):
                ...
                if k_fwd == 'KEY_T':
                    found_forward_n = ('KEY_T', v_fwd)
                    break
                elif k_fwd.startswith('start_'):
                    break
                elif k_fwd.startswith('end_'):
                    break
                elif k_fwd == 'KEY_D' or k_fwd == 'KEY_F':
                    break
            ...
            if found_back is not None and found_back[0] == 'start_i' and found_back[1] not in start_used:
                ...
                if start_found is not None and start_found not in start_used and found_forward_n is not None and found_forward_n[0] == 'KEY_T':
                        results_list.append({
                ...
                elif start_found is not None and start_found not in start_used and (found_forward_n is None or found_forward_n[0] != 'KEY_T'):
                    results_list.append({
...
def analyse_merged_ttl_tics_suppressed(
    ...
    while i < len(phase_events):
        ...
        if key == 'KEY_S':
            ...
            # ----- S then S -----
            if found_forward is not None and found_forward[0] == 'KEY_S':
                # results_list.append({
                #     "type": "S_then_S",
                #     "S_time": value,
                # })
            ...
            # ----- S then F -----
            if found_forward is not None and found_forward[0] == 'KEY_F':
                if found_forward[1] not in F_used and value not in S_used:
                    # check if there's a T just before this S
                    t_before_s = None
                    for j in range(i-1, -1, -1):
                        k, v = next(iter(phase_events[j].items()))
                        if k == 'KEY_T':
                            t_before_s = v
                            break
                        elif k in ('KEY_S', 'KEY_F'):
                            break

                    if t_before_s is not None and value - t_before_s <= max_t_before_S:
                        # T before S → it's T_then_S_then_F
                        results_list = [r for r in results_list if not (r.get('type') == 'T_before_S' and r.get('T_time') == t_before_s)]
                        results_list.append({
                        ...
                        S_used.add(value)
                        F_used.add(found_forward[1])
                        print(f"T at {t_before_s} then S at {value} then F at {found_forward[1]}")
                    else:
                        # no T before → regular S_then_F
                        results_list.append({
                            "type": "S_then_F",
                            "S_time": value,
                        })
        ...
        if key == 'KEY_T':
            ...
            if found_back is not None and found_back[0] == 'KEY_S':
        ...
        if key == 'KEY_F' :
            ...
            for j in range(i + 1, len(phase_events)):
                ...
                if k_fwd == 'T':
                    ...
                    break
                elif k_fwd == 'KEY_S' or k_fwd == 'KEY_F':
                    break
